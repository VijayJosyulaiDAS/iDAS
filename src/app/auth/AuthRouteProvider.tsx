...
import usePingAuth, {PingAuth} from './services/custom/usePingFed';
...
type AuthContext = {
	...
	pingService?: PingAuth<User, PingSignInPayload, SignUpPayload>;
	signOut?: () => void;
...
function AuthRouteProvider(props: AuthProviderProps) {
	...
	const jwtService = useJwtAuth({
		config: {
			...
			signInUrl: 'api/is_logged_in',
			signUpUrl: 'mock-api/auth/sign-up',
			tokenRefreshUrl: 'api/is_logged_in',
	...
	/**
	 * Pingfed auth service
	 */
	
	const pingService = usePingAuth({ 
		config: {
			tokenStorageKey: 'ping_jwt_access_token',
	...
	const isLoading = useMemo(
		() => jwtService?.isLoading || firebaseService?.isLoading || pingService?.isLoading,
		[jwtService?.isLoading, firebaseService?.isLoading, pingService?.isLoading]
	);
	...
	const isAuthenticated = useMemo(
		() => jwtService?.isAuthenticated || firebaseService?.isAuthenticated || pingService?.isLoading,
		[jwtService?.isAuthenticated, firebaseService?.isAuthenticated, pingService?.isLoading]
	);
