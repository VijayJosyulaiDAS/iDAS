...
function YesterdayProductionHeader({onUpload, jsonData}) {
    ...
    const handleUpload = async () => {
        ...
        try {
            setLoading(true)
            await axios.post(`${import.meta.env.VITE_LOCAL_BASE_URL}/create_yesterday_production`, formData, {
            }).then((response) =>{
                setUpdated(!updated)
    ...
    const handleExport = () =>{
        ...
        const worksheetData = jsonData.map(item => ({
            ...
            'Confirm Order Qty (BOUM)': item.confirmed_order_qty_boum ? item.confirmed_order_qty_boum.toString() : '',
            'Confirmed Order Qty': item.confirmed_order_qty ? item.confirmed_order_qty.toString() : '',
        }));
        ...
        saveAs(blob, "yesterday_production.xlsx");
...
export default YesterdayProductionHeader;